                           ...
                           void *typeSpecific)
{
  ...
  switch (pdu_type) {
    ...
    case FAPI_NR_RX_PDU_TYPE_SSB:
      ...
      memcpy(rx_ind->rx_indication_body[n_pdus - 1].ssb_pdu.pdu, ((fapiPbch_t *)typeSpecific)->decoded_output, sizeof(((fapiPbch_t *)typeSpecific)->decoded_output));
    ...
    case FAPI_NR_CSIRS_IND:
      memcpy(&rx_ind->rx_indication_body[n_pdus - 1].csirs_measurements, (fapi_nr_csirs_measurements_t *)typeSpecific, sizeof(*(fapi_nr_csirs_measurements_t *)typeSpecific));
      break;
    default:
      break;
  }
...
static inline uint16_t get_bw_scaling(uint16_t ofdm_symbol_size)
{
...
void ue_ta_procedures(PHY_VARS_NR_UE *ue, int slot_tx, int frame_tx)
{
  if (ue->mac_enabled == 1) {
    ...
    if (frame_tx == ul_time_alignment->ta_frame && slot_tx == ul_time_alignment->ta_slot) {
      ...
      LOG_D(PHY, "In %s: [UE %d] [%d.%d] Got timing advance command %u from MAC, new value is %d\n", __FUNCTION__, ue->Mod_id, frame_tx, slot_tx, ul_time_alignment->ta_command, ue->timing_advance);
...
void nr_ue_measurement_procedures(uint16_t l, PHY_VARS_NR_UE *ue, UE_nr_rxtx_proc_t *proc, uint8_t gNB_id, uint16_t slot)
{
  NR_DL_FRAME_PARMS *frame_parms = &ue->frame_parms;
...
static void nr_ue_pbch_procedures(uint8_t gNB_id, PHY_VARS_NR_UE *ue, UE_nr_rxtx_proc_t *proc, int estimateSz, struct complex16 dl_ch_estimates[][estimateSz], NR_UE_PDCCH_CONFIG *phy_pdcch_config)
{
  int ret = 0;
  DevAssert(ue);
  ...
  fapiPbch_t result;
  ret = nr_rx_pbch(ue, proc, estimateSz, dl_ch_estimates, ue->pbch_vars[gNB_id], &ue->frame_parms, gNB_id, (ue->frame_parms.ssb_index) & 7, SISO, phy_pdcch_config, &result);
...
#ifdef DEBUG_PHY_PROC
  LOG_D(PHY, "[UE %d] frame %d, slot %d, PBCH errors = %d, consecutive errors = %d!\n", ue->Mod_id, frame_rx, nr_slot_rx, ue->pbch_vars[gNB_id]->pdu_errors, ue->pbch_vars[gNB_id]->pdu_errors_conseq);
#endif
                           ...
                           int n_ss)
{
  ...
  for (int i = 0; i < dci_cnt; i++) {
    LOG_D(PHY,
          "[UE  %d] AbsSubFrame %d.%d, Mode %s: DCI %i of %d total DCIs found --> rnti %x : format %d\n",
          ue->Mod_id,
          frame_rx % 1024,
          nr_slot_rx,
          nr_mode_string[ue->UE_mode[gNB_id]],
          i + 1,
          dci_cnt,
...
int nr_ue_pdsch_procedures(PHY_VARS_NR_UE *ue, UE_nr_rxtx_proc_t *proc, int gNB_id, PDSCH_t pdsch, NR_UE_DLSCH_t *dlsch0, NR_UE_DLSCH_t *dlsch1)
{
  ...
  if (!dlsch1) {
    ...
    LOG_D(PHY,
          "[UE %d] PDSCH type %d active in nr_slot_rx %d, harq_pid %d (%d), rb_start %d, nb_rb %d, symbol_start %d, nb_symbols %d, DMRS mask %x, Nl %d\n",
          ue->Mod_id,
          pdsch,
          nr_slot_rx,
          harq_pid,
    ...
    if (ue->chest_time == 1) { // averaging time domain channel estimates
      nr_chest_time_domain_avg(&ue->frame_parms, ue->pdsch_vars[gNB_id]->dl_ch_estimates, dlsch0_harq->nb_symbols, dlsch0_harq->start_symbol, dlsch0_harq->dlDmrsSymbPos, pdsch_nb_rb);
    }
    ...
    for (m = s0; m < (s1 + s0); m++) {
      ...
      if (pdsch == PDSCH || pdsch == SI_PDSCH || pdsch == RA_PDSCH) {
        if (nr_rx_pdsch(ue, proc, pdsch, gNB_id, gNB_id_i, frame_rx, nr_slot_rx, m, first_symbol_flag, dual_stream_UE, i_mod, harq_pid) < 0)
          return -1;
      } else
        AssertFatal(1 == 0, "Not RA_PDSCH, SI_PDSCH or PDSCH\n");
...
bool nr_ue_dlsch_procedures(PHY_VARS_NR_UE *ue, UE_nr_rxtx_proc_t *proc, int gNB_id, PDSCH_t pdsch, NR_UE_DLSCH_t *dlsch0, NR_UE_DLSCH_t *dlsch1, int *dlsch_errors)
{
  if (dlsch0 == NULL)
  ...
  if (1) {
    ...
    dlsch0->harq_processes[harq_pid]->G =
        nr_get_G(dlsch0->harq_processes[harq_pid]->nb_rb, nb_symb_sch, nb_re_dmrs, dmrs_len, dlsch0->harq_processes[harq_pid]->Qm, dlsch0->harq_processes[harq_pid]->Nl);

    start_meas(&ue->dlsch_unscrambling_stats);
    nr_dlsch_unscrambling(pdsch_vars->llr[0], dlsch0->harq_processes[harq_pid]->G, 0, ue->frame_parms.Nid_cell, dlsch0->rnti);
    ...
    ret = nr_dlsch_decoding(ue, proc, gNB_id, pdsch_vars->llr[0], &ue->frame_parms, dlsch0, dlsch0->harq_processes[harq_pid], frame_rx, nb_symb_sch, nr_slot_rx, harq_pid, pdsch == PDSCH);
    ...
    if (is_cw1_active) {
      ...
      dlsch1->harq_processes[harq_pid]->G =
          nr_get_G(dlsch1->harq_processes[harq_pid]->nb_rb, nb_symb_sch, nb_re_dmrs, dmrs_len, dlsch1->harq_processes[harq_pid]->Qm, dlsch1->harq_processes[harq_pid]->Nl);
      start_meas(&ue->dlsch_unscrambling_stats);
      nr_dlsch_unscrambling(pdsch_vars->llr[1], dlsch1->harq_processes[harq_pid]->G, 0, ue->frame_parms.Nid_cell, dlsch1->rnti);
      stop_meas(&ue->dlsch_unscrambling_stats);
    ...
    if (ue->mac_enabled == 1) { // TODO: move this from PHY to MAC layer!
      ...
      if (ul_bwp) {
        if (mac->ULbwp[ul_bwp - 1] && mac->ULbwp[ul_bwp - 1]->bwp_Dedicated && mac->ULbwp[ul_bwp - 1]->bwp_Dedicated->pusch_Config && mac->ULbwp[ul_bwp - 1]->bwp_Dedicated->pusch_Config->choice.setup
        ...
        } else if (mac->ULbwp[ul_bwp - 1] && mac->ULbwp[ul_bwp - 1]->bwp_Common && mac->ULbwp[ul_bwp - 1]->bwp_Common->pusch_ConfigCommon
                   && mac->ULbwp[ul_bwp - 1]->bwp_Common->pusch_ConfigCommon->choice.setup && mac->ULbwp[ul_bwp - 1]->bwp_Common->pusch_ConfigCommon->choice.setup->pusch_TimeDomainAllocationList) {
      ...
      } else if (mac->scc_SIB && mac->scc_SIB->uplinkConfigCommon && mac->scc_SIB->uplinkConfigCommon->initialUplinkBWP.pusch_ConfigCommon
      ...
      if (ul_time_alignment->apply_ta == 1) {
        ...
        LOG_D(PHY,
              "Frame %d slot %d -- Starting UL time alignment procedures. TA update will be applied at frame %d slot %d\n",
              frame_rx,
              nr_slot_rx,
              ul_time_alignment->ta_frame,
              ul_time_alignment->ta_slot);
      }
    }
  }
  return dec;
  ...
  while (!oai_exit) {
    ...
    */
    ...
    if (nr_subframe_select(&ue->frame_parms, next_nr_slot_rx) != SF_UL) {
      ...
      front_end_fft(ue, pilot0, next_subframe_slot0, 0, 0);
    }
    ...
    for (int l = 1; l < ue->frame_parms.symbols_per_slot >> 1; l++) {
      ...
      front_end_chanEst(ue, l, slot1, 0);
      ue_measurement_procedures(l - 1, ue, proc, 0, slot1, 0, ue->mode);
    }
    // printf("AbsSubframe %d.%d ChanEst slot %d, symbol %d\n", frame_rx,nr_slot_rx,next_subframe_slot0,pilot0);
    front_end_chanEst(ue, pilot0, next_subframe_slot0, 0);
    ...
    if (ue->dlsch[proc->thread_id][gNB_id][0]->active == 1) {
      ...
      ue_pdsch_procedures(ue, proc, gNB_id, PDSCH, ue->dlsch[proc->thread_id][gNB_id][0], NULL, (ue->frame_parms.symbols_per_slot >> 1), ue->frame_parms.symbols_per_slot - 1, abstraction_flag);
    ...
    if ((ue->dlsch_SI[gNB_id]) && (ue->dlsch_SI[gNB_id]->active == 1)) {
      ue_pdsch_procedures(ue, proc, gNB_id, SI_PDSCH, ue->dlsch_SI[gNB_id], NULL, (ue->frame_parms.symbols_per_slot >> 1), ue->frame_parms.symbols_per_slot - 1, abstraction_flag);
    }

    // do procedures for P-RNTI
    if ((ue->dlsch_p[gNB_id]) && (ue->dlsch_p[gNB_id]->active == 1)) {
      ue_pdsch_procedures(ue, proc, gNB_id, P_PDSCH, ue->dlsch_p[gNB_id], NULL, (ue->frame_parms.symbols_per_slot >> 1), ue->frame_parms.symbols_per_slot - 1, abstraction_flag);
    }
    // do procedures for RA-RNTI
    if ((ue->dlsch_ra[gNB_id]) && (ue->dlsch_ra[gNB_id]->active == 1) && (UE_mode != PUSCH)) {
      ue_pdsch_procedures(ue, proc, gNB_id, RA_PDSCH, ue->dlsch_ra[gNB_id], NULL, (ue->frame_parms.symbols_per_slot >> 1), ue->frame_parms.symbols_per_slot - 1, abstraction_flag);
    }
...
int phy_procedures_nrUE_RX(PHY_VARS_NR_UE *ue, UE_nr_rxtx_proc_t *proc, uint8_t gNB_id, uint8_t dlsch_parallel, NR_UE_PDCCH_CONFIG *phy_pdcch_config, notifiedFIFO_t *txFifo)
{
  int frame_rx = proc->frame_rx;
  int nr_slot_rx = proc->nr_slot_rx;
    ...
    for (int ssb_index = 0; ssb_index < fp->Lmax; ssb_index++) {
      ...
      if ((curr_mask >> (31 - (ssb_index % 32))) & 0x01) {
        ...
        if (ssb_slot == nr_slot_rx || ssb_slot_2 == nr_slot_rx) {
          ...
          for (int i = 1; i < 4; i++) {
            nr_slot_fep(ue, proc, (ssb_start_symbol + i) % (fp->symbols_per_slot), nr_slot_rx);

            start_meas(&ue->dlsch_channel_estimation_stats);
            nr_pbch_channel_estimation(
                ue, estimateSz, dl_ch_estimates, dl_ch_estimates_time, proc, gNB_id, nr_slot_rx, (ssb_start_symbol + i) % (fp->symbols_per_slot), i - 1, ssb_index & 7, ssb_slot_2 == nr_slot_rx);
  ...
  for (int gNB_id = 0; gNB_id < ue->prs_active_gNBs; gNB_id++) {
    for (int rsc_id = 0; rsc_id < ue->prs_vars[gNB_id]->NumPRSResources; rsc_id++) {
      ...
      for (int i = 0; i < prs_config->PRSResourceRepetition; i++) {
        if ((((frame_rx * fp->slots_per_frame + nr_slot_rx) - (prs_config->PRSResourceSetPeriod[1] + prs_config->PRSResourceOffset) + prs_config->PRSResourceSetPeriod[0])
          ...
          for (int j = prs_config->SymbolStart; j < (prs_config->SymbolStart + prs_config->NumPRSSymbols); j++) {
            nr_slot_fep(ue, proc, (j % fp->symbols_per_slot), nr_slot_rx);
          }
  ...
  for (uint16_t l = 0; l < nb_symb_pdcch; l++) {
    start_meas(&ue->ofdm_demod_stats);
    nr_slot_fep(ue, proc, l, nr_slot_rx);
  }
  ...
  if (ue->dlsch[gNB_id][0]->active == 1) {
    ...
    ret_pdsch = nr_ue_pdsch_procedures(ue, proc, gNB_id, PDSCH, ue->dlsch[gNB_id][0], NULL);
  ...
  if ((ue->dlsch_SI[gNB_id]) && (ue->dlsch_SI[gNB_id]->active == 1)) {
    ...
    nr_ue_pdsch_procedures(ue, proc, gNB_id, SI_PDSCH, ue->dlsch_SI[gNB_id], NULL);

    nr_ue_dlsch_procedures(ue, proc, gNB_id, SI_PDSCH, ue->dlsch_SI[gNB_id], NULL, &ue->dlsch_SI_errors[gNB_id]);
  ...
  if ((ue->dlsch_p[gNB_id]) && (ue->dlsch_p[gNB_id]->active == 1)) {
    ...
    nr_ue_pdsch_procedures(ue, proc, gNB_id, P_PDSCH, ue->dlsch_p[gNB_id], NULL);

    nr_ue_dlsch_procedures(ue, proc, gNB_id, P_PDSCH, ue->dlsch_p[gNB_id], NULL, &ue->dlsch_p_errors[gNB_id]);
  ...
  if ((ue->dlsch_ra[gNB_id]) && (ue->dlsch_ra[gNB_id]->active == 1) && (ue->UE_mode[gNB_id] != PUSCH)) {
    ...
    nr_ue_pdsch_procedures(ue, proc, gNB_id, RA_PDSCH, ue->dlsch_ra[gNB_id], NULL);

    nr_ue_dlsch_procedures(ue, proc, gNB_id, RA_PDSCH, ue->dlsch_ra[gNB_id], NULL, &ue->dlsch_ra_errors[gNB_id]);
  ...
  if (ue->dlsch[gNB_id][0]->active == 1) {
    ...
    if (ret_pdsch >= 0)
      nr_ue_dlsch_procedures(ue, proc, gNB_id, PDSCH, ue->dlsch[gNB_id][0], dlsch1, &ue->dlsch_errors[gNB_id]);
  ...
  if (nr_slot_rx == 9) {
    ...
    LOG_D(PHY,
          ...
          frame_rx,
          ue->total_TBS[gNB_id],
          ue->total_TBS_last[gNB_id],
          (float)ue->bitrate[gNB_id] / 1000.0);
