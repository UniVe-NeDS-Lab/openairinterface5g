...
typedef enum { pss = 0, pbch = 1, si = 2 } sync_mode_t;
...
static void L1_nsa_prach_procedures(frame_t frame, int slot, fapi_nr_ul_config_prach_pdu *prach_pdu)
{
  ...
  uint8_t preamble_index = get_softmodem_params()->nsa ? mac->ra.rach_ConfigDedicated->cfra->resources.choice.ssb->ssb_ResourceList.list.array[0]->ra_PreambleIndex : mac->ra.ra_PreambleIndex;
...
static void process_queued_nr_nfapi_msgs(NR_UE_MAC_INST_t *mac, int sfn_slot)
{
  ...
  for (int i = 0; i < NR_MAX_HARQ_PROCESSES; i++) {
    LOG_D(NR_MAC,
          ...
          NFAPI_SFNSLOT2SLOT(mac->nr_ue_emul_l1.harq[i].active_ul_harq_sfn_slot),
...
static void check_nr_prach(NR_UE_MAC_INST_t *mac, nr_uplink_indication_t *ul_info, NR_PRACH_RESOURCES_t *prach_resources)
{
  ...
  if (mac->ra.ra_state != RA_SUCCEEDED) {
    AssertFatal(
        ul_config->number_pdus < sizeof(ul_config->ul_config_list) / sizeof(ul_config->ul_config_list[0]), "Number of PDUS in ul_config = %d > ul_config_list num elements", ul_config->number_pdus);
    fapi_nr_ul_config_prach_pdu *prach_pdu = &ul_config->ul_config_list[ul_config->number_pdus].prach_config_pdu;
    uint8_t nr_prach = nr_ue_get_rach(prach_resources, prach_pdu, ul_info->module_id, ul_info->cc_id, ul_info->frame_tx, ul_info->gNB_index, ul_info->slot_tx);
    if (nr_prach == 1) {
...
static void *NRUE_phy_stub_standalone_pnf_task(void *arg)
{
  ...
  while (!oai_exit) {
    ...
    if (is_nr_DL_slot(get_softmodem_params()->nsa ? mac->scc->tdd_UL_DL_ConfigurationCommon : mac->scc_SIB->tdd_UL_DL_ConfigurationCommon, ul_info.slot_rx)) {
      nr_ue_dl_indication(&mac->dl_info, &ul_time_alignment);
    }
    ...
    if (is_nr_UL_slot(get_softmodem_params()->nsa ? mac->scc->tdd_UL_DL_ConfigurationCommon : mac->scc_SIB->tdd_UL_DL_ConfigurationCommon, ul_info.slot_tx, mac->frame_type)) {
  ...
  if (tx_slot_type == NR_UPLINK_SLOT || tx_slot_type == NR_MIXED_SLOT) {
    if (UE->UE_mode[gNB_id] <= PUSCH) {
      if (get_softmodem_params()->usim_test == 0) {
        pucch_procedures_ue_nr(UE, gNB_id, proc);
      }

      LOG_D(PHY, "Sending Uplink data \n");
      nr_ue_pusch_common_procedures(UE, proc->nr_slot_tx, &UE->frame_parms, UE->frame_parms.nb_antennas_tx);
    }
...
void dummyWrite(PHY_VARS_NR_UE *UE, openair0_timestamp timestamp, int writeBlockSize)
{
  ...
  AssertFatal(writeBlockSize == UE->rfdevice.trx_write_func(&UE->rfdevice, timestamp, dummy_tx, writeBlockSize, UE->frame_parms.nb_antennas_tx, 4), "");
}
    ...
    for (int slot = 0; slot < UE->frame_parms.slots_per_subframe; slot++) {
      for (int i = 0; i < UE->frame_parms.nb_antennas_rx; i++) {
        if (toTrash)
          ...
          rxp[i] = ((void *)&UE->common_vars.rxdata[i][0]) + 4 * ((x * UE->frame_parms.samples_per_subframe) + UE->frame_parms.get_samples_slot_timestamp(slot, &UE->frame_parms, 0));
      }

      AssertFatal(UE->frame_parms.get_samples_per_slot(slot, &UE->frame_parms)
                      == UE->rfdevice.trx_read_func(&UE->rfdevice, timestamp, rxp, UE->frame_parms.get_samples_per_slot(slot, &UE->frame_parms), UE->frame_parms.nb_antennas_rx),
                  "");
...
void syncInFrame(PHY_VARS_NR_UE *UE, openair0_timestamp *timestamp)
{
  ...
  *timestamp += UE->frame_parms.get_samples_per_slot(1, &UE->frame_parms);
  for (int size = UE->rx_offset; size > 0; size -= UE->frame_parms.samples_per_subframe) {
    ...
    AssertFatal(unitTransfer == UE->rfdevice.trx_read_func(&UE->rfdevice, timestamp, (void **)UE->common_vars.rxdata, unitTransfer, UE->frame_parms.nb_antennas_rx), "");
...
void *UE_thread(void *arg)
{
  ...
  while (!oai_exit) {
    ...
    if (start_rx_stream == 0) {
      ...
      AssertFatal(UE->frame_parms.ofdm_symbol_size + UE->frame_parms.nb_prefix_samples0
                      == UE->rfdevice.trx_read_func(
                          &UE->rfdevice, &timestamp, (void **)UE->common_vars.rxdata, UE->frame_parms.ofdm_symbol_size + UE->frame_parms.nb_prefix_samples0, UE->frame_parms.nb_antennas_rx),
                  "");
    ...
    for (int i = 0; i < UE->frame_parms.nb_antennas_rx; i++)
      rxp[i] = (void *)&UE->common_vars.rxdata[i][firstSymSamp + UE->frame_parms.get_samples_slot_timestamp(slot_nr, &UE->frame_parms, 0)];

    for (int i = 0; i < UE->frame_parms.nb_antennas_tx; i++)
      txp[i] = (void *)&UE->common_vars.txdata[i][UE->frame_parms.get_samples_slot_timestamp(((slot_nr + DURATION_RX_TO_TX - NR_RX_NB_TH) % nb_slot_frame), &UE->frame_parms, 0)];
    ...
    AssertFatal(readBlockSize == UE->rfdevice.trx_read_func(&UE->rfdevice, &timestamp, rxp, readBlockSize, UE->frame_parms.nb_antennas_rx), "");
    ...
    if (slot_nr == (nb_slot_frame - 1)) {
      ...
      if (first_symbols > 0) {
        ...
        AssertFatal(first_symbols == UE->rfdevice.trx_read_func(&UE->rfdevice, &ignore_timestamp, (void **)UE->common_vars.rxdata, first_symbols, UE->frame_parms.nb_antennas_rx), "");
      } else
        LOG_E(PHY, "can't compensate: diff =%d\n", first_symbols);
    }

    curMsg->proc.timestamp_tx = timestamp + UE->frame_parms.get_samples_slot_timestamp(slot_nr, &UE->frame_parms, DURATION_RX_TO_TX) - firstSymSamp;
    ...
    writeTimestamp = timestamp + UE->frame_parms.get_samples_slot_timestamp(slot_nr, &UE->frame_parms, DURATION_RX_TO_TX - NR_RX_NB_TH) - firstSymSamp - openair0_cfg[0].tx_sample_advance
    ...
    if (flags || IS_SOFTMODEM_RFSIM)
      AssertFatal(writeBlockSize == UE->rfdevice.trx_write_func(&UE->rfdevice, writeTimestamp, txp, writeBlockSize, UE->frame_parms.nb_antennas_tx, flags), "");
