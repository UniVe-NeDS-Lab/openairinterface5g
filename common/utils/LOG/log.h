...
#define NUM_LOG_LEVEL                                                                                                              \
  6 /*!< \brief the number of message levels users have with LOG (OAILOG_DISABLE is not available to user as a level, so it is not \
       included)*/
/** @}*/
...
#define LOG_MASKMAP_INIT                                                                                                        \
  {                                                                                                                             \
    {"PRACH", DEBUG_PRACH}, {"RU", DEBUG_RU}, {"UE_PHYPROC", DEBUG_UE_PHYPROC}, {"LTEESTIM", DEBUG_LTEESTIM},                   \
        {"DLCELLSPEC", DEBUG_DLCELLSPEC}, {"ULSCH", DEBUG_ULSCH}, {"RRC", DEBUG_RRC}, {"PDCP", DEBUG_PDCP}, {"DFT", DEBUG_DFT}, \
        {"ASN1", DEBUG_ASN1}, {"CTRLSOCKET", DEBUG_CTRLSOCKET}, {"SECURITY", DEBUG_SECURITY}, {"NAS", DEBUG_NAS},               \
        {"RLC", DEBUG_RLC}, {"DLSCH_DECOD", DEBUG_DLSCH_DECOD}, {"UE_TIMING", UE_TIMING},                                       \
    {                                                                                                                           \
      NULL, -1                                                                                                                  \
    }                                                                                                                           \
  }
...
@param multiVec create new file or append to existing (useful for writing multiple vectors to same file. Just call the function
multiple times with same file name and with this parameter set to 1)
*/
...
#define LOG_CONFIG_HELP_OPTIONS                                                           \
  " list of comma separated options to enable log module behavior. Available options: \n" \
...
/*   optname                               help                                          paramflags         XXXptr defXXXval type
 * numelt */
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
// clang-format off
...
// debugging macros
#define LOG_F                                                                                                            \
  LOG_I /* because  LOG_F was originaly to dump a message or buffer but is also used as a regular level...., to dump use \
           LOG_DUMPMSG */

// #  if T_TRACER
// #include "T.h"
// /* per component, level dependent macros */
// #    define LOG_E(c, x...) do { T(T_LEGACY_ ## c ## _ERROR, T_PRINTF(x))  ; if (T_stdout) { if( g_log->log_component[c].level >=
// OAILOG_ERR     ) logRecord_mt(__FILE__, __FUNCTION__, __LINE__,c, OAILOG_ERR, x)     ;} } while (0) #    define LOG_W(c, x...) do
// { T(T_LEGACY_ ## c ## _WARNING, T_PRINTF(x)); if (T_stdout) { if( g_log->log_component[c].level >= OAILOG_WARNING )
// logRecord_mt(__FILE__, __FUNCTION__, __LINE__,c, OAILOG_WARNING, x) ;} } while (0) #    define LOG_A(c, x...) do { T(T_LEGACY_ ##
// c ## _INFO, T_PRINTF(x))   ; if (T_stdout) { if( g_log->log_component[c].level >= OAILOG_ANALYSIS) logRecord_mt(__FILE__,
// __FUNCTION__, __LINE__,c, OAILOG_ANALYSIS, x);} } while (0) #    define LOG_I(c, x...) do { T(T_LEGACY_ ## c ## _INFO,
// T_PRINTF(x))   ; if (T_stdout) { if( g_log->log_component[c].level >= OAILOG_INFO    ) logRecord_mt(__FILE__, __FUNCTION__,
// __LINE__,c, OAILOG_INFO, x)    ;} } while (0) #    define LOG_D(c, x...) do { T(T_LEGACY_ ## c ## _DEBUG, T_PRINTF(x))  ; if
// (T_stdout) { if( g_log->log_component[c].level >= OAILOG_DEBUG   ) logRecord_mt(__FILE__, __FUNCTION__, __LINE__,c, OAILOG_DEBUG,
// x)   ;} } while (0) #    define LOG_T(c, x...) do { T(T_LEGACY_ ## c ## _TRACE, T_PRINTF(x))  ; if (T_stdout) { if(
// g_log->log_component[c].level >= OAILOG_TRACE   ) logRecord_mt(__FILE__, __FUNCTION__, __LINE__,c, OAILOG_TRACE, x)   ;} } while
// (0) #    define VLOG(c,l, f, args) do { if (T_stdout) { if( g_log->log_component[c].level >= l  ) vlogRecord_mt(__FILE__,
// __FUNCTION__, __LINE__,c, l, f, args)   ;} } while (0)
...
// #    define LOG_M(file, vector, data, len, dec, format) do { write_file_matlab(file, vector, data, len, dec, format, 0);}
// while(0)/* */
...
#define LOG_UM(file, vector, data, len, dec, format)            \
  do {                                                          \
    write_file_matlab(file, vector, data, len, dec, format, 0); \
  } while (0)
#define LOG_UI(c, x...)                                                \
...
#define LOG_UDUMPMSG(c, b, s, f, x...) \
  do {                                 \
    log_dump(c, b, s, f, x);           \
  } while (0) /* */
#define LOG_MM(file, vector, data, len, dec, format)            \
  do {                                                          \
    write_file_matlab(file, vector, data, len, dec, format, 1); \
  } while (0)
/** @}*/
...
#define LOG_EXIT(c) \
  do {              \
    LOG_END(c);     \
    return;         \
  } while (0) /*!< \brief Macro to log a message with severity TRACE when exiting a function */
#define LOG_RETURN(c, r)                                                     \
  do {                                                                       \
    LOG_T(c, "Leaving %s (rc = %08lx)\n", __FUNCTION__, (unsigned long)(r)); \
    return (r);                                                              \
  } while (0) /*!< \brief Macro to log a function exit, including integer value, then to return a value to the calling function */
