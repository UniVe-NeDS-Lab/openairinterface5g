...
#include <openair2/COMMON/o1_messages_types.h>
...
#define FOREACH_TASK(TASK_DEF)                                       \
  ...
  TASK_DEF(TASK_O1, TASK_PRIORITY_MED, 200, NULL, NULL)              \
...
static const task_info_t tasks_info[] = {FOREACH_TASK(TASK_DEF)};

#define TASK_ENUM(TaskID, pRIO, qUEUEsIZE, FuNc, ThreadFunc) TaskID,
//! Tasks id of each task
typedef enum { FOREACH_TASK(TASK_ENUM) } task_id_t;
...
/** \brief Wait for SIGINT/SIGTERM signals to unblock ITTI.
   This function should be called from the main thread after having created all ITTI tasks. If handler is NULL, a default handler is
 installed. \param handler a custom signal handler. To unblock, it should call itti_wait_tasks_unblock()
 **/
...
int timer_setup(uint32_t interval_sec,
                uint32_t interval_us,
                task_id_t task_id,
                instance_t instance,
                timer_type_t type,
